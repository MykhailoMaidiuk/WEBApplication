...
def get_books():
    ...
    page_size = int(request.args.get('page_size', 48))  # Výchozí počet knih na stránce je 50
...
def add_favorite():
    data = request.get_json()
    isbn13 = data.get('isbn13', {}).get('isbn13')

    if not isbn13:
        return jsonify({"error": "ISBN13 je povinné"}), 400

    print(f"Přidávám knihu s ISBN13: {isbn13}")

    # Zkontrolujeme, zda je isbn13 řetězec (nebo jiný očekávaný typ)
    if not isinstance(isbn13, str):
        return jsonify({"error": "ISBN13 musí být řetězec"}), 400
    ...
    try:
        ...
        print(f"Kniha nalezena: {book.title}")

        # Kontrola, zda už kniha není v oblíbených
        favorite = session.query(FavoriteBook).filter_by(user_id=current_user.id, book_isbn13=isbn13).first()
        if favorite:
            return jsonify({"message": "Kniha již je v oblíbených"}), 200

        # Přidání knihy do oblíbených
        new_favorite = FavoriteBook(user_id=current_user.id, book_isbn13=isbn13)
        session.add(new_favorite)
        session.commit()

        # Získání aktualizovaného seznamu oblíbených knih
        favorites = session.query(Book).join(FavoriteBook).filter(FavoriteBook.user_id == current_user.id).all()
        favorites_list = [book.as_dict() for book in favorites]

        return jsonify(favorites_list), 201

    except Exception as e:
        print("Chyba:", e)
        session.rollback()
        return jsonify({"error": "Chyba při přidávání oblíbené knihy"}), 500
    finally:
        session.close()
...
def remove_from_favorites():
    ...
    try:
        favorite = session.query(FavoriteBook).filter_by(user_id=current_user.id, book_isbn13=isbn13).first()
        if not favorite:
            return jsonify({"message": "Kniha nebyla nalezena v oblíbených"}), 404

        session.delete(favorite)
        session.commit()

        # Aktualizujeme seznam oblíbených knih
        favorites = session.query(Book).join(FavoriteBook).filter(FavoriteBook.user_id == current_user.id).all()
        favorites_list = [book.as_dict() for book in favorites]
        ...
        print("Error:", e)
        session.rollback()
        return jsonify({"error": "Chyba při odebírání oblíbené knihy"}), 500
    finally:
        session.close()
...
def favorites():
    ...
    session = Session()
    try:
        # Získáme oblíbené knihy uživatele
        query = session.query(Book).join(FavoriteBook).filter(FavoriteBook.user_id == current_user.id)
        ...
        # Výpočet OFFSETu a aplikace LIMITu pro stránkování
        offset = (page - 1) * page_size
        query = query.offset(offset).limit(page_size)

        # Načteme oblíbené knihy
        favorite_books = query.all()
        books_list = [book.as_dict() for book in favorite_books]

        # Počet všech oblíbených knih


        # Vrátíme odpověď s knihami a stránkováním
        return jsonify({
            'books': books_list,
            'totalBooks': total_books,
            'totalPages': (total_books // page_size) + (1 if total_books % page_size > 0 else 0),
